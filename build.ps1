...
if ($Install) {
    ...
    $modDataDir = Join-Path $modsDir 'FullAuto'
...
if ($Package) {
    ...
    $zip = Join-Path $PSScriptRoot 'FullAuto.zip'

    if (Test-Path $stage) { Remove-Item $stage -Recurse -Force }
    New-Item -ItemType Directory -Force -Path (Join-Path $stage 'FullAuto') | Out-Null
    Copy-Item $dll -Destination $stage -Force
    Copy-Item (Join-Path $PSScriptRoot 'manifest.json') -Destination (Join-Path $stage 'FullAuto') -Force
